...
void quick_sort(Container& c, int64_t start, int64_t end, const Partition_functor& p) {
  if (start >= end) {
    return;
  }
  auto pivot = p(c, start, end);
  // At this point, the pivot value is in its final sorted position at index 'begin'
...
{
  return sort(c, [](auto& con, auto start, auto end){ return details::partition(con, start, end); });
}
