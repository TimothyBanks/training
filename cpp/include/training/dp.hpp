...
*/
template <typename T>
int32_t brute_force(const std::vector<T>& left, size_t lend, const std::vector<T>& right, size_t rend) {
  if (lend == 0 || rend == 0) {
    return 0;
  } else if (left[lend-1] == right[rend-1]) {
    return 1 + brute_force(left, lend - 1, right, rend - 1);
  } else {
    return std::max(brute_force(left, lend, right, rend - 1), brute_force(left, lend - 1, right, rend));
  }
}

using lookup_table = std::unordered_map<size_t, std::unordered_map<size_t, int32_t>>;
template <typename T>
int32_t memoization(const std::vector<T>& left, size_t lend, const std::vector<T>& right, size_t rend, lookup_table& table) {
  if (lend == 0 || rend == 0) {
    return 0;
  }
  ...
  if(left[lend-1] == right[rend-1]) {
    ...
    result = std::max(memoization(left, lend, right, rend - 1, table), memoization(left, lend - 1, right, rend, table));
  }
...
template <typename T>
int32_t solve(solve_by by, const std::vector<T>& left, const std::vector<T>& right) {
  switch(by) {
...
template <typename T>
int32_t memoization(const std::vector<T>& v, size_t index, lookup_table& lookup) {
  if (index == 0) {
    return 1;
  }
...
using lookup_table = std::unordered_map<size_t, int32_t>;
template <typename T>
int32_t memoization(const std::vector<T>& coins, size_t index, T amount, lookup_table& table) {
  if (amount == 0) { 
    return 0;
  }
...
using lookup_table = std::unordered_map<size_t, std::unordered_map<size_t, int32_t>>;
template <typename T>
int32_t memoization(const std::vector<T>& v, size_t index, int32_t k, lookup_table& table) {
  if (k == 0) { 
    return 1;
  }
...
int32_t bottom_up(const std::vector<T>& v, int32_t k) {
  auto mins = std::unordered_map<int32_t, std::pair<T, std::unordered_set<size_t>>>{};
...
void run_tests() {
  ...
  std::cout << "LCS brute force " << std::to_string(longest_common_subsequence::solve(solve_by::brute_force, left, right)) << std::endl;
  std::cout << "LCS memoization" << std::to_string(longest_common_subsequence::solve(solve_by::memoization, left, right)) << std::endl;
  std::cout << "LCS bottom up/tabulation" << std::to_string(longest_common_subsequence::solve(solve_by::bottom_up, left, right)) << std::endl;
  ...
  std::cout << "LIS brute force " << std::to_string(longest_increasing_subsequence::solve(solve_by::brute_force, sequence1)) << std::endl;
  std::cout << "LIS brute force " << std::to_string(longest_increasing_subsequence::solve(solve_by::brute_force, sequence2)) << std::endl;
  std::cout << "LIS brute force " << std::to_string(longest_increasing_subsequence::solve(solve_by::brute_force, sequence3)) << std::endl;
  std::cout << "LIS memoization " << std::to_string(longest_increasing_subsequence::solve(solve_by::memoization, sequence1)) << std::endl;
  std::cout << "LIS memoization " << std::to_string(longest_increasing_subsequence::solve(solve_by::memoization, sequence2)) << std::endl;
  std::cout << "LIS memoization " << std::to_string(longest_increasing_subsequence::solve(solve_by::memoization, sequence3)) << std::endl;
  std::cout << "LIS bottom up/tabulation " << std::to_string(longest_increasing_subsequence::solve(solve_by::bottom_up, sequence1)) << std::endl;
  std::cout << "LIS bottom up/tabulation " << std::to_string(longest_increasing_subsequence::solve(solve_by::bottom_up, sequence2)) << std::endl;
  std::cout << "LIS bottom up/tabulation " << std::to_string(longest_increasing_subsequence::solve(solve_by::bottom_up, sequence3)) << std::endl;

  auto coins = std::vector<int32_t>{1, 2, 5};
  std::cout << "COINS brute force " << std::to_string(make_coins::solve(solve_by::brute_force, coins, 11)) << std::endl;
  std::cout << "COINS memoization " << std::to_string(make_coins::solve(solve_by::memoization, coins, 11)) << std::endl;
  std::cout << "COINS bottom up/tabulation " << std::to_string(make_coins::solve(solve_by::bottom_up, coins, 11)) << std::endl;

  auto sequence4 = std::vector<int32_t>{198, 76, 544, 123, 154, 675}; // 9348
  auto sequence5 = std::vector<int32_t>{11, 8, 5, 7, 5, 100}; // 1400
  std::cout << "Min Product brute force " << std::to_string(minimum_product::solve(solve_by::brute_force, sequence4, 2)) << std::endl;
  std::cout << "Min Product brute force " << std::to_string(minimum_product::solve(solve_by::brute_force, sequence5, 4)) << std::endl;
  std::cout << "Min Product memoization " << std::to_string(minimum_product::solve(solve_by::memoization, sequence4, 2)) << std::endl;
  std::cout << "Min Product memoization " << std::to_string(minimum_product::solve(solve_by::memoization, sequence5, 4)) << std::endl;
  std::cout << "Min Product bottom up/tabulation " << std::to_string(minimum_product::solve(solve_by::bottom_up, sequence4, 2)) << std::endl;
  std::cout << "Min Product bottom up/tabulation " << std::to_string(minimum_product::solve(solve_by::bottom_up, sequence5, 4)) << std::endl;
  std::cout << "Min Product heap " << std::to_string(minimum_product::details::min_heap(sequence4, 2)) << std::endl;
  std::cout << "Min Product heap " << std::to_string(minimum_product::details::min_heap(sequence5, 4)) << std::endl;
}
