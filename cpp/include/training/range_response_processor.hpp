...
class range_response_processor {
  ...
  using block_confirmations = std::unordered_map<block, std::shared_ptr<std::atomic<uint64_t>>>;
...
public:
  range_response_processor(uint64_t confirmation_threshold, confirmed_block_functor&& confirmation_functor) 
    : m_confirmation_threshold{confirmation_threshold},
      m_confirmed_block_functor{std::forward<confirmed_block_functor>(confirmation_functor)} {}
  ...
  auto process_range(uint64_t start_height, const std::vector<block>& blocks) -> void 
  {
    ...
    for (auto block_height = start_height; block_height < start_height + blocks.size(); ++block_height) {
      ...
      {
        // This is a happy path in that the block height has already been encountered and the block
        // has already been encountered at this block height.  This should be the majority of the runtime,
        // so we can do all this behind a reader lock.
        ...
        if (auto active_it = m_active_blocks.find(block_height); active_it != std::end(m_active_blocks)) {
          if (auto block_it = active_it->second.find(current_block); block_it != std::end(active_it->second)) {
              // Since confirmation count is atomic, we can do all of this behind the shared locks.
              found = true;
      ...
      if (is_confirmed(block_height)) {
        ...
        // We do this again because another thread may have confirmed the block at this
        // block height before we get here.
        continue;
      }

      // There is this edge case where the confirmation might be something like 0 or 1.  That case should be taken
      // care of if we allow the insertion to happen first.
      ...
      if (active_it == std::end(m_active_blocks)) {
        ...
        m_active_blocks.emplace(block_height, block_confirmations{{current_block, std::make_shared<std::atomic<uint64_t>>(1)}});
        ...
        active_it->second.emplace(current_block, std::make_shared<std::atomic<uint64_t>>(1));
        confirmation_count = 1;
      }
      ...
      // No iterators beyond this point. They will be invalidated by the mutations to the data structures.
      auto confirmed_lock = std::unique_lock{m_confirmed_mutex};

      // block is no longer active.
      m_active_blocks.erase(block_height);

      // The first one will always win here.  So if we encounter another confirmed block, it will be ignored.
