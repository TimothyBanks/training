 // Implement a simple calculator that takes an arithmetic formula as input and evaluates it. Operations include +,-,*,/ and ()
...
int64_t evaluate2(const std::vector<std::string>& tokens, size_t& start, bool short_circuit) {
  auto s = std::stack<std::string>{};
  ...
  for (; start < tokens.size();) {
    ...
    if (short_circuit && s.size() == 1 && s.top().find_first_of("()*/+-") == std::string::npos) {
      break;
    }
  }
