...
void run_bounded_buffer_test() {
  ...
  for (size_t i = 0; i < how_many_consumers; ++i) {
    consumers.emplace_back([&](auto index){
      while (true) {
        auto data = buffer.pop();
        {
          auto lock = std::lock_guard<std::mutex>{write_mutex};
          std::cout << "from consumer " << index << " read " << data << std::endl;
        }
  ...
  for (size_t i = 0; i < how_many_producers; ++i) {
    producers.emplace_back([&](auto index){
      auto message = std::string{"hello from producer "} + std::to_string(index);
      while (true) {
        buffer.push(message);
